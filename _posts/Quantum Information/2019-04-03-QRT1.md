...
1. 为了定义态与过程, 我们首先考虑我们基础 Hilbert 空间 $\mathcal{H}$.
    我个人比较喜欢 Watrous 去年新出的书 [2] 中使用的定义,
    如果我之后想把量子计算也整理一下的话我再介绍这个. 为了简单起见,
    这里我们直接把它看作 $n$ 维复欧几里得空间 $\CC^n$.
2. 态 $\rho\in\mathcal{H}$ 是 Hilbert 空间上的密度算子.
    $\mathcal{H}$ 上所有密度算子的集合记作 $\mathcal{S}(\mathcal{H})$,
    它是有界算子空间 $\mathcal{B}(\mathcal{H})$ 的一个子集.
    我们把所有 Hilbert 空间上的密度算子集合记作 $\mathcal{S}$.
3. 量子信道 $\Phi: \mathcal{B}(\mathcal{H}\_{in})\to
    \mathcal{B}(\mathcal{H}\_{out})$
    是所有的 CPTP 超算子. 由于我们在讨论具体问题的时候经常放松 CPTP 的要求,
    我们不为量子信道分配一个符号. 但是我们把两个空间中所有可能的超算子记为
    $\mathcal{B}(\mathcal{H}\_{in}, \mathcal{H}\_{out})$. 所有超算子记为 $\mathcal{B}$.
...
我们一般把经典状态当作免费的. $\mathcal{O}\subseteq\mathcal{B}$ 为自由算子,
即所有我们可以自由使用的操作. $\mathcal{F}(\mathcal{H}),
\mathcal{O}(\mathcal{H}\_{in},\mathcal{H}\_{out})$ 为定义在对应空间上的自由态与自由算子.
对于资源理论, 我们只有三条限制:

1. $\forall \Phi\in\mathcal{O}(\mathcal{H}_A,\mathcal{H}_B),
    \Lambda\in\mathcal{O}(\mathcal{H}_B,\mathcal{H}_C)$, 我们要求
    $\Lambda\circ\Phi\in\mathcal{O}(\mathcal{H}_A,\mathcal{H}_C)$.
2. $\text{id}\in\mathcal{O}(\mathcal{H},\mathcal{H})$.
3. $\forall\rho\in\mathcal{F}(\mathcal{H}\_{in}),
    \Phi\in\mathcal{O}(\mathcal{H}\_{in},\mathcal{H}\_{out})$, 我们要求
    $\Phi(\rho)\in\mathcal{F}(\mathcal{H}\_{out})$.

这三个条件使得我们的资源理论变成了一个对称幺半范畴, 或者物理中习惯叫*对称张量范畴*.

注意: $\CC$ 本身也是一个 Hilbert 空间, 而 $1$ 是 $\CC$ 上面唯一合法的密度算子.
$\mathcal{O}(\mathbb{C},\mathcal{H})$ 根据定义等价于所有使得
$\rho\in\mathcal{F}(\mathcal{H})$ 的超算子 $\Phi(1)\to\rho$ 的集合.
