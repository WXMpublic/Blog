...
[GPG](https://gnupg.org/) is also known as GnuPG, stands for GNU Privacy Guard. It is a built in program for most Linux systems and [GitBash](https://git-scm.com/about) for Windows.
...
Note that **the flag `-e` can be used with signature, which also encrypt the file at the same time**.
